...
export default function Notifications() {
  ...
  useEffect(() => {
    async function loadNotifications() {
      ...
      try {
        ...
        console.error('Error loading dean notifications:', err);
      } finally {
        setLoading(false);
      }
    }
  ...
  const filtered = useMemo(() => {
    return notifications.filter(n => {
      ...
      if (activeFilter === 'Grades') return n.type === 'grades' || n.type === 'override';
      if (activeFilter === 'Evaluations') return n.type === 'eval';
      if (activeFilter === 'Risk Alerts') return n.type === 'risk';
      return true;
    });
      ...
      <div className="p-3.5 sm:p-6 md:p-8 overflow-y-auto flex-1 space-y-3.5 sm:space-y-6 md:space-y-8">
        ...
        <div className="flex items-center justify-between gap-2 border-b border-slate-200 pb-3">
          <div className="flex items-center gap-1.5 sm:gap-2 overflow-x-auto no-scrollbar py-0.5">
            {['All', 'Unread', 'Grades', 'Evaluations', 'Risk Alerts'].map(filter => (
              <button
                key={filter}
        ...
        <div className="space-y-2.5 sm:space-y-4">
          {filtered.length > 0 ? (
            ...
            <div className="text-center py-12 sm:py-16 bg-white border border-slate-200 rounded-2xl p-4 shadow-sm">
              ...
              <p className="text-xs text-slate-400 mt-1">No dean notifications found matching your filter selection.</p>
            </div>
          )}
        </div>
