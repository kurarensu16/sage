...
export default function Notifications() {
  ...
  useEffect(() => {
    async function loadNotifications() {
      if (!user) return;
      const cacheKey = `office_notifs_${user.id}`;
  ...
  const markAllRead = async () => {
    ...
    try {
      ...
      setCachedData(`office_notifs_${user.id}`, updated);
      refreshUnreadCount?.();
    } catch (err) {
  ...
  const markAsRead = async (id) => {
    try {
      ...
      setNotifications(updated);
      if (user) setCachedData(`office_notifs_${user.id}`, updated);
      refreshUnreadCount?.();
    } catch (err) {
  ...
  const deleteSelected = async () => {
    ...
    try {
      ...
      setSelectedIds(new Set());
      if (user) setCachedData(`office_notifs_${user.id}`, remaining);
      refreshUnreadCount?.();
    } catch (err) {
  ...
  return (
    <>
      ...
      <div className="p-3.5 sm:p-6 md:p-8 overflow-y-auto flex-1 space-y-3.5 sm:space-y-6 md:space-y-8">
        ...
        {/* Filters Tab Pills & Selection Bar */}
