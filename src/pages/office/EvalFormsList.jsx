  ...
  return (
    <>
      ...
      <div className="p-8 overflow-y-auto flex-1 space-y-6">
        ...
        {/* Templates Feed (Mobile Cards + Desktop Table) */}
        
        {/* Mobile View Card Feed */}
        <div className="md:hidden space-y-3">
          {filteredTemplates.length > 0 ? (
            filteredTemplates.map((t) => (
              <div key={t.id} className="bg-white rounded-2xl border border-slate-200/90 p-4 shadow-2xs space-y-3 text-left">
                <div className="flex items-start justify-between gap-2">
                  <div className="flex items-start gap-2.5 min-w-0">
                    <div className="p-2 bg-sage-50 text-sage-600 rounded-xl shrink-0 mt-0.5">
                      <FileText className="h-4 w-4" />
                    </div>
                    <div className="min-w-0">
                      <h4 className="font-bold text-slate-900 font-display text-sm truncate">{t.title}</h4>
                      <p className="text-[11px] text-slate-400 font-mono mt-0.5">Created: {formatDate(t.createdDate)}</p>
                    </div>
                  </div>
                  <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-slate-100 text-slate-700 border border-slate-200 font-mono shrink-0">
                    {t.criteria ? t.criteria.length : 0} Criteria
                  </span>
                </div>

                <div className="pt-2 border-t border-slate-100 flex items-center justify-between text-xs">
                  <span className="text-slate-500 text-[11px]">Author: {t.author}</span>
                  <div className="flex items-center gap-1.5">
                    <button 
                      onClick={() => navigate(`/office/evalbuilder?id=${t.id}`)}
                      className="px-3 py-1.5 text-xs font-semibold text-slate-700 bg-slate-50 hover:bg-slate-100 rounded-lg border border-slate-200 flex items-center gap-1 cursor-pointer"
                    >
