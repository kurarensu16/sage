...
| **College Office** | Roster verification & clearance compliance | Department CSV imports, Certificate of Registration (COR) verification, student evaluation completion auditing, physical & digital clearance sign-off. | `users`, `sections`, `enrollments`, `evaluation_responses`, `activity_logs` |
| **Faculty** | Class record management, score entry & resubmissions | Class record setup, score entry with full titles, grade resubmission requests with evidence files, advisory FDA handling, Dean-released evaluation review. | `class_records`, `grade_components`, `component_scores`, `posted_grades`, `grade_resubmission_requests`, `ai_faculty_predictions` |
| **Student** | Grades review, faculty feedback & clearance | Midterm/Final grade tracking, activity breakdowns, on-time survey evaluation for clearance sign-off, late survey submission for grade unlock. | `posted_grades`, `component_scores`, `evaluation_responses`, `evaluation_ratings`, `evaluation_comments`, `ai_student_recommendations`, `notifications` |
| **Dean** | Oversight, quality assurance & resubmission approvals | Monitoring grade posting, unlock/resubmission approval with evidence review, Dean evaluation release control toggles, Dual-Channel & Retaliation-Drift analytics. | `class_records`, `posted_grades`, `unlock_requests`, `grade_resubmission_requests`, `evaluation_windows`, `evaluation_ratings`, `ai_faculty_predictions` |
